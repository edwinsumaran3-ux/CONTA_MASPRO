...
import { SaleFormEnterprise, type SaleSubmitPayload } from './SaleFormEnterprise';
...
export const EnterpriseWorkspace = () => {
  ...
  const loadJournal = async (bearerToken: string, period = { year: 2026, month: 5 }) => {
    ...
    const response = await fetch(`${API_BASE}/ledger/journal?year=${period.year}&month=${period.month}&limit=250`, {
      headers: authHeaders(bearerToken, currentTenantId),
    });
  ...
  const postSale = async (salePayload: SaleSubmitPayload) => {
    ...
    try {
      const tokenResponse = await fetch(`${API_BASE}/auth/dev-token`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tenant_id: tenantId, user_id: USER_ID, role: 'ADMIN' }),
      });
      if (!tokenResponse.ok) throw new Error(await tokenResponse.text());
      const tokenData = await tokenResponse.json();
      const saleToken = tokenData.access_token;
      if (!saleToken) throw new Error('No hay token de seguridad para registrar venta.');

      const payload = {
        tenant_id: tenantId,
        year: 2026,
        month: 5,
        invoice_id: `${salePayload.form.serie}-${salePayload.form.number}`,
        customer_ruc: salePayload.form.customerRuc,
        customer_name: salePayload.customerName,
        entry_date: salePayload.issueDate || new Date().toISOString().slice(0, 10),
        doc_type: '01',
        serie: salePayload.form.serie,
        number: salePayload.form.number,
        subtotal: Number.parseFloat(salePayload.subtotal),
        igv: Number.parseFloat(salePayload.igv),
        total: Number.parseFloat(salePayload.total),
        currency: 'PEN',
        revenue_account: salePayload.form.revenueAccount || '7011',
        cost_center: salePayload.form.costCenter || 'LIM-COM',
        line_items: salePayload.items.map((item) => ({
          ...
          line_subtotal: Number.parseFloat(item.lineSubtotal),
        })),
        account_lines: salePayload.accountLines.map((l) => ({
          account_code: l.accountCode,
          account_name: l.accountName,
          cost_center: l.costCenter,
          debit: Number.parseFloat(l.debit),
          credit: Number.parseFloat(l.credit),
          line_type: l.lineType,
          tax_treatment: l.taxTreatment,
        })),
        audit_metadata: salePayload.auditMetadata,
      };

      console.log('CONTA_PRO postSale payload:', payload);
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('Error ledger invoice:', errorText);
        throw new Error(errorText);
      }

      const responsePayload = await response.json();
      console.info('CONTA_PRO /ledger/invoice respuesta:', responsePayload);
      if (responsePayload.diagnostics) {
        console.info('CONTA_PRO persistencia venta:', {
          database_url: responsePayload.diagnostics.database_url,
          insert_or_update_executed: responsePayload.diagnostics.insert_or_update_executed,
          commit_executed: responsePayload.diagnostics.commit_executed,
          commit_confirmed: responsePayload.diagnostics.commit_confirmed,
          select_returned_rows: responsePayload.diagnostics.select_returned_rows,
          select_after_save: responsePayload.diagnostics.select_after_save,
        });
      }

      const postedMessage = `Venta ${salePayload.form.serie}-${salePayload.form.number} posteada con cuentas y asiento contable.`;
      setStatusMessage(postedMessage);
      setToken(saleToken);

      try {
        const [saleYear, saleMonth] = String(salePayload.issueDate || '').split('-').map((part) => Number.parseInt(part, 10));
        const refreshedRows = await loadJournal(saleToken, {
          year: Number.isFinite(saleYear) ? saleYear : 2026,
          month: Number.isFinite(saleMonth) ? saleMonth : 5,
        });
        console.info('CONTA_PRO SELECT /ledger/journal despues de guardar venta:', refreshedRows.slice(0, 5));
        const diagnostics = responsePayload.diagnostics;
        if (diagnostics) {
          setStatusMessage(
            `${postedMessage} Commit: ${diagnostics.commit_confirmed ? 'OK' : 'no confirmado'}. BD: ${
              diagnostics.database_url || 'no reportada'
            }. SELECT journal_entries=${
              diagnostics.select_returned_rows?.journal_entries ?? 0
            }, financial_documents=${diagnostics.select_returned_rows?.financial_documents ?? 0}.`
          );
        }
      } catch (journalError) {
