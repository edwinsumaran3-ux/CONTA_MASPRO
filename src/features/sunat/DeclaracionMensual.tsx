 ...
 * Genera PLE/SIRE, resumen IGV, PDF imprimible
 */
...
const printPDF = (titulo: string, html: string) => {
  ...
  win.document.write(`
    ...
    <body>${html}<footer>CONTA_PRO Enterprise · Generado el ${new Date().toLocaleString('es-PE')} · Documento de uso interno</footer></body>
    </html>`);
  win.document.close();
  win.focus();
...
export const DeclaracionMensual: React.FC = () => {
      ...
      for (const e of entries) {
        ...
        if (mod === 'BILLING') {
          ...
          for (const l of lines) {
            ...
            if (code.startsWith('4011')) ventasIgv += Number(l.credit || 0);
          }
        }
        if (mod === 'PURCHASING') {
          nFactC++;
          for (const l of lines) {
            ...
            if (code.startsWith('40111') || code.startsWith('40112')) comprasIgv += Number(l.debit || 0);
          }
        }
      }
  ...
  const generarLibros = async () => {
    setLoading(true); setEstado('EN_PROCESO'); setMsg('Generando libros PLE/SIRE...');
    try {
      const h = await headers();
      ...
      if (r.ok) {
        ...
        setLibros([
          { nombre: 'LE080100_2026_05.txt', codigo: 'LE0801', filas: resumen?.num_facturas_c || 0, archivo: 'Registro de Compras' },
          { nombre: 'LE140100_2026_05.txt', codigo: 'LE1401', filas: resumen?.num_facturas_v || 0, archivo: 'Registro de Ventas' },
          { nombre: 'LE050100_2026_05.txt', codigo: 'LE0501', filas: (resumen?.num_facturas_v || 0) + (resumen?.num_facturas_c || 0), archivo: 'Libro Diario' },
        ]);
        setEstado('GENERADO');
      ...
      setLibros([
        { nombre: 'LE080100.txt', codigo: 'LE0801', filas: resumen?.num_facturas_c || 0, archivo: 'Registro de Compras' },
        { nombre: 'LE140100.txt', codigo: 'LE1401', filas: resumen?.num_facturas_v || 0, archivo: 'Registro de Ventas' },
        { nombre: 'LE050100.txt', codigo: 'LE0501', filas: (resumen?.num_facturas_v || 0) + (resumen?.num_facturas_c || 0), archivo: 'Libro Diario' },
      ]);
    } finally { setLoading(false); }
  };
  ...
  const imprimirResumen = () => {
    if (!resumen) return;
    const igvLabel = resumen.igv_saldo >= 0 ? 'IGV A PAGAR' : 'SALDO A FAVOR';
    const igvClass = resumen.igv_saldo >= 0 ? 'pagar' : 'favor';
    const html = `
      <h1>CONTA_PRO — Declaración Mensual SUNAT</h1>
      <p class="sub">Período: ${MESES[mes-1]} ${anio} &nbsp;|&nbsp; RUC Contador: — &nbsp;|&nbsp; Generado: ${new Date().toLocaleString('es-PE')}</p>
      <div class="igv-box">
        <div class="kpi"><div class="kpi-label">Total Ventas (Base + IGV)</div><div class="kpi-value">S/ ${fmt(resumen.ventas_total)}</div></div>
        <div class="kpi"><div class="kpi-label">Total Compras (Base + IGV)</div><div class="kpi-value">S/ ${fmt(resumen.compras_total)}</div></div>
        <div class="kpi"><div class="kpi-label">${igvLabel}</div><div class="kpi-value ${igvClass}">S/ ${fmt(Math.abs(resumen.igv_saldo))}</div></div>
      </div>
      <h2>Registro de Ventas — ${resumen.num_facturas_v} comprobantes</h2>
      <table>
        <tr><th>Concepto</th><th class="right">Base Imponible</th><th class="right">IGV 18%</th><th class="right">Total</th></tr>
        <tr><td>Ventas gravadas</td><td class="right">S/ ${fmt(resumen.ventas_base)}</td><td class="right">S/ ${fmt(resumen.ventas_igv)}</td><td class="right">S/ ${fmt(resumen.ventas_total)}</td></tr>
        <tr class="total"><td>TOTAL VENTAS</td><td class="right">S/ ${fmt(resumen.ventas_base)}</td><td class="right">S/ ${fmt(resumen.ventas_igv)}</td><td class="right">S/ ${fmt(resumen.ventas_total)}</td></tr>
      </table>
      <h2>Registro de Compras — ${resumen.num_facturas_c} comprobantes</h2>
      <table>
        <tr><th>Concepto</th><th class="right">Base Imponible</th><th class="right">IGV Crédito Fiscal</th><th class="right">Total</th></tr>
        <tr><td>Compras gravadas</td><td class="right">S/ ${fmt(resumen.compras_base)}</td><td class="right">S/ ${fmt(resumen.compras_igv)}</td><td class="right">S/ ${fmt(resumen.compras_total)}</td></tr>
        <tr class="total"><td>TOTAL COMPRAS</td><td class="right">S/ ${fmt(resumen.compras_base)}</td><td class="right">S/ ${fmt(resumen.compras_igv)}</td><td class="right">S/ ${fmt(resumen.compras_total)}</td></tr>
      </table>
      <h2>Determinación del IGV</h2>
      <table>
        <tr><th>Concepto</th><th class="right">Importe S/</th></tr>
        <tr><td>IGV Débito Fiscal (ventas)</td><td class="right">S/ ${fmt(resumen.igv_debito)}</td></tr>
        <tr><td>IGV Crédito Fiscal (compras)</td><td class="right">(S/ ${fmt(resumen.igv_credito)})</td></tr>
        <tr class="total"><td>${igvLabel}</td><td class="right ${igvClass}">S/ ${fmt(Math.abs(resumen.igv_saldo))}</td></tr>
      </table>
      <p style="font-size:10px;color:#666;margin-top:8px;">
        * Este resumen es de uso interno. Para la declaración oficial use el PDT 621 o SUNAT en línea.<br/>
        * Verifique los comprobantes en el Registro de Ventas y Compras antes de presentar.
      </p>`;
    printPDF(`Declaración SUNAT ${period}`, html);
  };
    ...
    try {
      ...
      const rows = entries.map(e => `
        <tr>
          ...
          <td class="right">S/ ${fmt(e.total_debit || 0)}</td>
          <td class="right">S/ ${fmt(e.total_credit || 0)}</td>
          <td>${e.sunat_status || 'POSTED'}</td>
        </tr>`).join('');
      ...
      const html = `
        <h1>CONTA_PRO — Libro Diario</h1>
        <p class="sub">Período: ${MESES[mes-1]} ${anio} &nbsp;|&nbsp; ${entries.length} asientos &nbsp;|&nbsp; Generado: ${new Date().toLocaleString('es-PE')}</p>
        <table>
          <tr><th>Fecha</th><th>Glosa</th><th>Módulo</th><th class="right">Debe S/</th><th class="right">Haber S/</th><th>Estado</th></tr>
          ${rows}
          <tr class="total">
            <td colspan="3">TOTALES</td>
            <td class="right">S/ ${fmt(totalD)}</td>
            <td class="right">S/ ${fmt(totalC)}</td>
            <td></td>
          </tr>
        </table>`;
    ...
    try {
      ...
      const rows = ventas.map((e,i) => {
        ...
        return `<tr>
          ...
          <td class="right">S/ ${fmt(e.total_credit||0)}</td>
          <td class="right">S/ ${fmt(Number(e.total_credit||0)*0.18/1.18)}</td>
          <td class="right">S/ ${fmt(e.total_credit||0)}</td>
        </tr>`;
      }).join('');

      const base = ventas.reduce((s,e) => s + Number(e.total_credit||0)/1.18, 0);
      const igv  = ventas.reduce((s,e) => s + Number(e.total_credit||0)*0.18/1.18, 0);
      const tot  = ventas.reduce((s,e) => s + Number(e.total_credit||0), 0);

      const html = `
        <h1>CONTA_PRO — Registro de Ventas e Ingresos</h1>
        <p class="sub">Período: ${MESES[mes-1]} ${anio} &nbsp;|&nbsp; ${ventas.length} comprobantes &nbsp;|&nbsp; LE 14</p>
        <table>
          <tr><th>N°</th><th>Fecha</th><th>Tipo</th><th>Serie-Número</th><th>RUC Cliente</th><th class="right">Base S/</th><th class="right">IGV S/</th><th class="right">Total S/</th></tr>
          ${rows || '<tr><td colspan="8" style="text-align:center;color:#aaa">Sin comprobantes en el período</td></tr>'}
          <tr class="total"><td colspan="5">TOTALES</td><td class="right">S/ ${fmt(base)}</td><td class="right">S/ ${fmt(igv)}</td><td class="right">S/ ${fmt(tot)}</td></tr>
        </table>`;
  ...
  const imprimirRegistroCompras = async () => {
    ...
    try {
      ...
      const rows = compras.map((e,i) => {
        const first = (e.lines||[])[0] || {};
        const igvLine = (e.lines||[]).find((l:any) => String(l.account_code||'').startsWith('40111')) || {};
        ...
        return `<tr>
          ...
          <td>${first.document_series||''}-${first.document_number||''}</td>
          <td>${first.partner_ruc||''}</td>
          <td class="right">S/ ${fmt(base)}</td>
          <td class="right">S/ ${fmt(igvVal)}</td>
          <td class="right">S/ ${fmt(total)}</td>
        </tr>`;
      }).join('');
      ...
      const html = `
        <h1>CONTA_PRO — Registro de Compras</h1>
        <p class="sub">Período: ${MESES[mes-1]} ${anio} &nbsp;|&nbsp; ${compras.length} comprobantes &nbsp;|&nbsp; LE 08</p>
        <table>
          <tr><th>N°</th><th>Fecha</th><th>Tipo</th><th>Serie-Número</th><th>RUC Proveedor</th><th class="right">Base S/</th><th class="right">IGV S/</th><th class="right">Total S/</th></tr>
          ${rows || '<tr><td colspan="8" style="text-align:center;color:#aaa">Sin comprobantes en el período</td></tr>'}
          <tr class="total"><td colspan="5">TOTALES</td><td class="right">S/ ${fmt(base)}</td><td class="right">S/ ${fmt(igv)}</td><td class="right">S/ ${fmt(tot)}</td></tr>
        </table>`;
  ...
  return (
    <div style={{ height:'100%', overflowY:'auto', background:C.bg, color:C.text, fontFamily:"'Segoe UI', Arial, sans-serif", padding:'18px 20px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:18, flexWrap:'wrap', gap:10 }}>
        <div>
          <h1 style={{ margin:0, fontSize:17, fontWeight:900, color:C.text }}>📋 Declaración Mensual SUNAT</h1>
          <p style={{ margin:'3px 0 0', fontSize:12, color:C.muted }}>PLE/SIRE · Registro Ventas · Registro Compras · Libro Diario · PDFs</p>
        </div>
      ...
      {resumen && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:16 }}>
          <KpiBox label="Total Ventas" value={`S/ ${fmt(resumen.ventas_total)}`} color={C.green} sub={`${resumen.num_facturas_v} comprobantes · Base S/ ${fmt(resumen.ventas_base)}`} />
          <KpiBox label="Total Compras" value={`S/ ${fmt(resumen.compras_total)}`} color={C.blue} sub={`${resumen.num_facturas_c} comprobantes · Crédito fiscal S/ ${fmt(resumen.compras_igv)}`} />
          <KpiBox
            label={resumen.igv_saldo >= 0 ? '⚠ IGV A PAGAR' : '✓ Saldo a favor'}
            value={`S/ ${fmt(Math.abs(resumen.igv_saldo))}`}
            color={resumen.igv_saldo >= 0 ? C.red : C.green}
            sub={`Débito S/ ${fmt(resumen.igv_debito)} — Crédito S/ ${fmt(resumen.igv_credito)}`}
          />
        </div>
      )}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:16 }}>
        ...
        <div style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:12, padding:'16px 18px' }}>
          <p style={{ margin:'0 0 12px', fontSize:13, fontWeight:800, color:C.text }}>📦 Generar libros PLE/SIRE</p>
          ...
          <button type="button" onClick={generarLibros} disabled={loading} style={{
            ...
            {loading ? '⏳ Generando...' : '⚡ Generar libros PLE/SIRE'}
          </button>
          {estado === 'GENERADO' && (
            <div style={{ marginTop:8, padding:'8px 12px', background:`${C.green}12`, border:`1px solid ${C.green}33`, borderRadius:8, fontSize:11, color:C.green }}>
              ✅ {libros.length} archivos listos para descarga y presentación SUNAT
            </div>
          )}
        </div>
        ...
        <div style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:12, padding:'16px 18px' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            <BtnPDF label="Resumen Declaración IGV"   icon="📊" onClick={imprimirResumen} />
            <BtnPDF label="Registro de Ventas (LE 14)" icon="📄" onClick={imprimirRegistroVentas} />
            <BtnPDF label="Registro de Compras (LE 08)"icon="📄" onClick={imprimirRegistroCompras} />
            <BtnPDF label="Libro Diario (LE 05)"        icon="📒" onClick={imprimirLibroDiario} />
          </div>
      ...
      <div style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:12, padding:'14px 18px' }}>
        ...
        <div style={{ display:'flex', gap:0, position:'relative' }}>
          {[
            ...
            { label:'2. Generar PLE/SIRE',      desc:'Genera los libros electrónicos',       done: estado !== 'PENDIENTE' && estado !== 'EN_PROCESO' },
            { label:'3. Imprimir reportes',      desc:'Descarga PDFs de respaldo',            done: false },
            { label:'4. Presentar SUNAT',         desc:'Ingresa al portal SOL de SUNAT',      done: estado === 'PRESENTADO' },
          ].map((s, i, arr) => (
        ...
        <div style={{ marginTop:16, padding:'10px 14px', background:`${C.yellow}10`, border:`1px solid ${C.yellow}33`, borderRadius:8 }}>
          <p style={{ margin:0, fontSize:11, color:C.muted }}>
            ...
            Ingresa al portal{' '}
            <strong style={{ color:C.accent }}>SOL de SUNAT (sunat.gob.pe)</strong>{' '}
            con tu clave SOL, usa el PDT 621 o el Formulario Virtual 621 con los datos del resumen generado aquí.
          </p>
        </div>
      </div>
    </div>
