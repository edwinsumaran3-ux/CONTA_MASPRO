...
// ── Panel Mis Negocios extraído como componente para cumplir con las Reglas de Hooks ──
const NegociosPanel = ({ onClose }: { onClose: () => void }) => {
    ...
    <div style={{ position:'fixed', inset:0, zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(2,8,18,0.75)', backdropFilter:'blur(4px)' }}
      ...
      <div style={{ background:'#080f1f', border:'1px solid #1e3a5f', borderRadius:14, padding:'24px 28px', width:'100%', maxWidth:560, maxHeight:'85vh', overflowY:'auto', boxShadow:'0 32px 80px rgba(0,0,0,0.8)', position:'relative' }}
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
          ...
          <button type="button" onClick={onClose} style={{ background:'none', border:'none', color:'#6e93b8', fontSize:18, cursor:'pointer', padding:'4px 8px' }}>✕</button>
        </div>
            ...
            {companies.map((c: Company) => {
              ...
              return (
                <div key={c.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 14px', borderRadius:10, cursor:'pointer', transition:'all 0.15s', background: isActive ? 'rgba(56,189,248,0.1)' : 'rgba(11,21,37,0.8)', border:`1px solid ${isActive ? '#38bdf8' : '#1e3a5f'}` }}
                  onClick={() => { setCompany(c.id); onClose(); }}>
                  <div>
...
const [accountDetailOpen, setAccountDetailOpen] = useState(false);
  ...
  useEffect(() => {
    const tokenTenant = tokenTenantId(token);
    // Solo renovar si podemos leer el tenant del token Y no coincide con la empresa activa.
    // Si tokenTenant === '' el JWT no expone tenant_id → no hacer nada (evita loop infinito).
    if (token && tokenTenant && tokenTenant !== getTenantId()) {
  ...
  return (
    <div className="enterprise-shell">
      ...
      {activePanel === 'NEGOCIOS' && <NegociosPanel onClose={() => setActivePanel(null)} />}
