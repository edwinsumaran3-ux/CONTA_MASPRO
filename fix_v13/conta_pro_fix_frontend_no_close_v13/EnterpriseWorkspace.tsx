...
export const EnterpriseWorkspace = () => {
  ...
  const postPurchase = async (purchasePayload?: PurchaseSubmitPayload) => {
    ...
    try {
      ...
      // CONTA_PRO FIX:
      // La compra ya fue posteada. Si falla la recarga del Libro Diario,
      // NO debe cerrar el modal ni mostrar que la compra falló.
      try {
        await loadJournal(purchaseToken);
        setActivePanel(null);
      } catch (journalError) {
        console.warn('CONTA_PRO loadJournal after purchase warning:', journalError);
        setStatusMessage(`${postedMessage} Pendiente refrescar Libro Diario: ${journalError instanceof Error ? journalError.message : 'error desconocido'}`);
      }
    } catch (error) {
