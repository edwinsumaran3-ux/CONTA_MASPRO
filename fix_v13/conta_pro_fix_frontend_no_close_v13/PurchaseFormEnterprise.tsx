...
const normalizeLineType = (value?: string): AccountingLineType => {
  ...
  if (clean === 'REGULATED_CHARGE') return 'REGULATED_CHARGE';
  if (clean === 'REGULATED_DISCOUNT') return 'REGULATED_DISCOUNT';
...
const isPublicOrRegulatedReceipt = (supplier = '', itemsLike: Array<{ description?: string; lineType?: string }> = []) => {
  const text = `${supplier} ${itemsLike.map((item) => `${item.description || ''} ${item.lineType || ''}`).join(' ')}`.toUpperCase();
  return /HIDRANDINA|ELECTRONORTE|ENEL|LUZ DEL SUR|ELECTRICIDAD|SEDAPAL|EPS|SUNASS|OSINERGMIN|MUNICIPALIDAD|GOBIERNO|MINISTERIO|SUNAT|ESSALUD|FOSE|FISE|APORTE LEY|ALUMBRADO/.test(text);
};

const isFakeOcrAdjustment = (description = '') => {
  const text = description.toUpperCase();
  return /AJUSTE POR DIFERENCIA|DIFERENCIA DE LECTURA|CONCEPTO REGULADO NO IDENTIFICADO|AJUSTE OCR/.test(text);
};

const shouldHideCorrectedWarning = (message = '') => {
  const text = message.toUpperCase();
  return /IGV DECLARADO|NO CORRESPONDE AL 18|NO ES EL 18|BASE IMPONIBLE|AUDITOR[IÍ]A|DIFERENCIA MAYOR A TOLERANCIA|AJUSTE AUTOMATICO|AJUSTE AUTOMÁTICO|SUMA DE LOS CONCEPTOS/.test(text);
};

const sanitizePublicReceiptWarnings = (messages: string[], isPublicReceipt: boolean, reconciliationStatus?: string) => {
  if (!isPublicReceipt || String(reconciliationStatus || '').toUpperCase() !== 'OK') {
    return messages;
  }
  return messages.filter((message) => !shouldHideCorrectedWarning(String(message)));
};
...
export const PurchaseFormEnterprise = ({ form, onFormChange, tenantId, onClose, onSubmit }: PurchaseFormEnterpriseProps) => {
  ...
  const subtotalItems = useMemo(
    () => items.reduce((acc, item) => item.lineType === 'INFO_ONLY' ? acc : acc + toNumber(item.lineSubtotal), 0),
    [items],
  );
  ...
  const applyGeminiPayload = (payload: GeminiPurchaseResponse) => {
    ...
    let mappedItems = (payload.items || []).map((raw) => {
    ...
    const isPublicReceipt = isPublicOrRegulatedReceipt(nextSupplierName, mappedItems);
    const hasVisibleRounding = mappedItems.some((item) => isRoundingLine(item.description, item.code));

    if (isPublicReceipt && hasVisibleRounding) {
      mappedItems = mappedItems.filter((item) => !isFakeOcrAdjustment(item.description));
    }
    ...
    const mappedAccountLines: ExplicitAccountLine[] = (payload.account_lines || []).map((raw) => ({
      ...
      if (!line.accountCode || (toNumber(line.debit) === 0 && toNumber(line.credit) === 0)) return false;
      if (line.lineType === 'INFO_ONLY') return false;
      if (isPublicReceipt && hasVisibleRounding && isFakeOcrAdjustment(`${line.accountName} ${line.auditNote || ''}`)) return false;
      return true;
    });

    const mergedWarnings = sanitizePublicReceiptWarnings([
      ...(payload.warnings || []),
      ...(payload.audit_metadata?.ocr_warnings || []),
      ...(payload.audit_metadata?.reconciliation_notes || []),
    ], isPublicReceipt, payload.reconciliation_status);
    ...
    setAiTaxWarnings(sanitizePublicReceiptWarnings(payload.audit_metadata?.tax_warnings || [], isPublicReceipt, payload.reconciliation_status));
    setAiLegalWarnings(sanitizePublicReceiptWarnings(payload.audit_metadata?.legal_warnings || [], isPublicReceipt, payload.reconciliation_status));
    setAiAccountingWarnings(sanitizePublicReceiptWarnings(payload.audit_metadata?.accounting_warnings || [], isPublicReceipt, payload.reconciliation_status));
  ...
  const handleSubmit = async () => {
    ...
    const publicReceiptSubmit = isPublicOrRegulatedReceipt(supplierName, items);
    const visibleRoundingSubmit = items.some((item) => isRoundingLine(item.description, item.code));

    const normalizedItems = items
      .filter((item) => item.lineType !== 'INFO_ONLY')
      .filter((item) => !(publicReceiptSubmit && visibleRoundingSubmit && isFakeOcrAdjustment(item.description)))
      .map((item) => ({
      code: item.code,
    ...
    const submitPayload: PurchaseSubmitPayload = {
      ...
      accountLines: accountingLines
        .filter((line) => line.lineType !== 'INFO_ONLY')
        .filter((line) => !(publicReceiptSubmit && visibleRoundingSubmit && isFakeOcrAdjustment(`${line.accountName} ${line.auditNote || ''}`)))
        .map((line) => ({
        accountCode: line.accountCode,
        accountName: line.accountName,
