    ...
    async def post_journal(self, payload: dict) -> JournalEntry:
        ...
        async with self.uow_factory(tenant_id) as uow:
            ...
            entry = JournalEntry(
                ...
                row_hash="PENDING",
                created_by=_safe_uuid(payload.get("user_id")),
            )
            for line in lines:
            ...
            if payload.get("financial_document"):
                ...
                if "metadata_json" in document_payload:
                    document_payload["metadata_json"] = _json_safe(document_payload["metadata_json"])
            ...
            await repo.add_audit(AuditLog(
                ...
                actor_user_id=_safe_uuid(payload.get("user_id")),
            ...
            for event in payload.get("outbox_events", []):
                await repo.add_outbox(OutboxEvent(
                    ...
                    payload=_json_safe(event.get("payload", {"journal_entry_id": str(entry.id), "trace_id": payload["trace_id"]})),
                    status="PENDING",
                    attempts=0,
    ...
    async def post_invoice(self, invoice_data: dict) -> JournalEntry:
        ...
        async with self.uow_factory(tenant_id) as uow:
            ...
            entry = JournalEntry(
                ...
                previous_hash=previous_hash,
                row_hash="PENDING",
                created_by=_safe_uuid(invoice_data.get("user_id")),
            )
            for line in lines:
                line.entry_id = entry.id
            ...
            await repo.add_audit(AuditLog(
                ...
                actor_user_id=_safe_uuid(invoice_data.get("user_id")), ip_address=invoice_data.get("ip_address"), user_agent=invoice_data.get("user_agent")
            ))
            await repo.add_outbox(OutboxEvent(
                tenant_id=tenant_id, topic="sunat.invoice.post", aggregate_type="JournalEntry", aggregate_id=str(entry.id),
                payload=_json_safe({"invoice": invoice_data, "journal_entry_id": str(entry.id), "financial_document_id": str(document.id), "trace_id": invoice_data["trace_id"]}),
