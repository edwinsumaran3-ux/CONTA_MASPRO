...
FASE 1 - LECTURA VISUAL PIXEL POR PIXEL OBLIGATORIA:
- Antes de clasificar contablemente, transcribe el bloque "IMPORTES FACTURADOS" exactamente en orden vertical.
- No calcules ni corrijas importes en esta fase. Copia el numero impreso.
- Devuelve raw_visual_lines con: orden, texto_visible, importe_visible, bloque.
- Bloques permitidos: BEFORE_SUBTOTAL, SUBTOTAL, IGV, AFTER_IGV, TOTAL, INFO_AFTER_TOTAL.
- SUB TOTAL impreso manda como base gravada si esta visible.
- IGV / I.G.V. / Imp. Gral. a las Ventas impreso manda como IGV fiscal.
- TOTAL RECIBO / TOTAL A PAGAR impreso manda como total.
- Errores OCR frecuentes que debes evitar:
  88.88 NO es 89.88.
  16.00 NO es 15.00.
  0.04 NO es 0.54.
  -0.01 NO es 0.01 positivo.
- Si el numero no es claro, no inventes: usa requires_visual_review=true y agrega ocr_warning.
- FOSE/FISE que aparece despues de TOTAL RECIBO como nota informativa no debe entrar doble al asiento si el total ya cuadra sin FOSE.
- Despues de transcribir visualmente, recien aplica clasificacion contable y tributaria.

REGLA DE ORDEN EN RECIBOS ELECTRICOS:
- Conceptos antes de SUB TOTAL forman la base gravada del recibo cuando el documento lo muestra asi.
- Conceptos despues del IGV o despues del TOTAL RECIBO no inflan IGV salvo discriminacion expresa.
- Saldo por redondeo y diferencia de redondeo se usan para reconciliar el total impreso.
- Aporte Ley puede ser cargo no afecto/posterior al IGV si el recibo lo muestra despues del IGV.
- Aporte FOSE/FISE debajo del TOTAL RECIBO es informativo si el total ya cuadra sin sumarlo.
...
{{
  ...
  "raw_visual_lines": [
    {
      "orden": 1,
      "texto_visible": "",
      "importe_visible": "0.00",
      "bloque": "BEFORE_SUBTOTAL|SUBTOTAL|IGV|AFTER_IGV|TOTAL|INFO_AFTER_TOTAL"
    }
  ],
  "items": [
    {{
      ...
      "igv_amount": "0.00",
      "total_line": "0.00",
      "line_type": "EXPENSE_OR_ASSET|PRIOR_BALANCE|ADVANCE_PAYMENT|LATE_FEE|ROUNDING|REGULATED_CHARGE|INFO_ONLY",
      "account_code": "",
      "account_name": "",
      "cost_center": "",
      "tax_treatment": "",
  ...
  "account_lines": [
    {{
      ...
      "debit": "0.00",
      "credit": "0.00",
      "line_type": "EXPENSE_OR_ASSET|TAX|PAYABLE|ROUNDING|PRIOR_BALANCE|ADVANCE_PAYMENT|LATE_FEE|REGULATED_CHARGE|INFO_ONLY|WITHHOLDING|DETRACTION|PERCEPTION",
      "tax_treatment": "",
      "audit_note": ""
    }}
  ],
...
def _desc_has(item: dict[str, Any], *tokens: str) -> bool:
    desc = _norm_upper(item.get("description"))
    return any(token.upper() in desc for token in tokens)


def _set_item_amount(item: dict[str, Any], amount: Decimal) -> None:
    value = _money_str(amount)
    item["line_subtotal"] = value
    item["unit_price"] = value
    item["total_line"] = value


def _printed_money(data: dict[str, Any], *keys: str) -> Decimal | None:
    for key in keys:
        value = data.get(key)
...
def _reconcile_visual_utility_receipt(
    data: dict[str, Any],
    ...
    """Corrige errores OCR tipicos en recibos de servicios cuando el bloque visible permite cuadrar."""
    supplier = _norm_upper(data.get("supplier_name"))
    text_all = " ".join([supplier] + [_norm_upper(i.get("description")) for i in items])
    is_electric = any(token in text_all for token in ["HIDRANDINA", "ELECTRIC", "ENERGIA", "ENERGÍA", "ALUMBRADO", "FOSE", "FISE"])
    if not is_electric:
        return

    total_read = _printed_money(data, "printed_total", "total_read_from_document", "total")
    if total_read is None or total_read == 0:
        return

    base_items = [
        item for item in items
        if _desc_has(item, "CARGO FIJO", "REPOSICION", "REPOSICIÓN", "MANTENIMIENTO", "ENERGIA ACTIVA", "ENERGÍA ACTIVA", "ALUMBRADO PUBLICO", "ALUMBRADO PÚBLICO")
    ]
    base_from_items = sum((_item_amount(i) for i in base_items), Decimal("0.00")).quantize(Decimal("0.01"))

    current_subtotal = _money(data.get("subtotal"))
    printed_subtotal = _printed_money(data, "printed_subtotal", "subtotal_impreso")
    if printed_subtotal is not None:
        data["subtotal"] = _money_str(printed_subtotal)
        current_subtotal = printed_subtotal
    elif base_from_items > 0 and (current_subtotal == 0 or abs(current_subtotal - base_from_items) <= Decimal("1.01")):
        if current_subtotal != base_from_items:
            ocr_warnings.append(f"Subtotal OCR corregido de {current_subtotal} a {base_from_items} usando conceptos antes de SUB TOTAL.")
        data["subtotal"] = _money_str(base_from_items)
        current_subtotal = base_from_items

    current_igv = _money(data.get("igv"))
    printed_igv = _printed_money(data, "printed_igv", "igv_impreso")
    if printed_igv is not None:
        data["igv"] = _money_str(printed_igv)
        current_igv = printed_igv
    elif current_subtotal > 0:
        igv_from_base = (current_subtotal * Decimal("0.18")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        if current_igv == 0 or abs(current_igv - igv_from_base) <= Decimal("1.01"):
            if current_igv != igv_from_base:
                tax_warnings.append(f"IGV OCR corregido de {current_igv} a {igv_from_base} por SUB TOTAL visible; validar contra IGV impreso.")
            data["igv"] = _money_str(igv_from_base)
            current_igv = igv_from_base

    fose_items = [item for item in items if _desc_has(item, "FOSE", "FISE")]
    saldo_redondeo = sum((_item_amount(i) for i in items if _desc_has(i, "SALDO POR REDONDEO")), Decimal("0.00"))
    aporte_ley = sum((_item_amount(i) for i in items if _desc_has(i, "APORTE LEY", "LEY NRO", "LEY 28749")), Decimal("0.00"))

    diff_items = [i for i in items if _desc_has(i, "DIFERENCIA DE REDONDEO", "DIFERENCIA POR REDONDEO")]
    diff_current = sum((_item_amount(i) for i in diff_items), Decimal("0.00"))
    expected_diff = (total_read - (current_subtotal + current_igv + saldo_redondeo + aporte_ley)).quantize(Decimal("0.01"))

    if diff_items and abs(diff_current - expected_diff) <= Decimal("1.01"):
        if diff_current != expected_diff:
            ocr_warnings.append(f"Diferencia de redondeo OCR corregida de {diff_current} a {expected_diff}.")
        _set_item_amount(diff_items[0], expected_diff)

    new_diff = sum((_item_amount(i) for i in diff_items), Decimal("0.00"))
    total_without_fose = (current_subtotal + current_igv + saldo_redondeo + aporte_ley + new_diff).quantize(Decimal("0.01"))
    if fose_items and abs(total_without_fose - total_read) <= Decimal("0.02"):
        for item in fose_items:
            ...
            item["tax_treatment"] = "FOSE/FISE mostrado como dato informativo posterior al total; no se registra doble si el TOTAL RECIBO ya cuadra."
            item["ai_reason"] = "FOSE/FISE aparece fuera de la suma del total del recibo."
        reconciliation_notes.append("FOSE/FISE tratado como informativo porque el total cuadra sin sumarlo.")
...
def _normalize_ai_response(data: dict[str, Any]) -> dict[str, Any]:
    ...
    _reconcile_visual_utility_receipt(data, items, warnings, tax_warnings, accounting_warnings, ocr_warnings, reconciliation_notes)
...
Analiza el archivo como comprobante empresarial peruano. Primero haz lectura visual real del archivo adjunto, no uses datos anteriores.
Usa criterio contable, tributario, legal-documentario y auditoria despues de transcribir el bloque visual.
